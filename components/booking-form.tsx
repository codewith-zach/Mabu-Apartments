...
export function BookingForm({ roomTypeId, price, title }: { roomTypeId: string; price: number; title: string }) {
  ...
  const calculateTotalPrice = useCallback((from: Date, to: Date) => {
  ...
  const handleDateSelect = useCallback((range: DateRange | undefined) => {
    ...
    if (range.from && range.to) {
      const isRangeAvailable = !unavailableDates.some(unavailableDate => 
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            {dateRange.from && dateRange.to && (
              <div className="space-y-2 pt-4">
                ...
                <p className="text-sm text-gray-500">
                  {nights > 0 &&
                    `${nights} night${nights !== 1 ? 's' : ''} at ₦${price.toLocaleString()} per night`
                  }
                </p>
              </div>
            )}
